...
class Segmenter:
    def __init__(self, sample_rate=16000, silence_seconds=.8, max_seconds=12):
    ...
    def feed(self, samples: np.ndarray):
        ...
        threshold = max(.012, min(self.noise*3.5, .05))
        speech = rms > threshold
        if not self.frames:
            if not speech:
                self.noise = .96*self.noise + .04*rms
        ...
        output = np.concatenate(self.frames)[:int(self.max_seconds*self.sample_rate)] if self.voiced >= .25 else None
        self.reset()
        return output
