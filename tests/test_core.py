...
from pet.events import MouseTracker, crop_rect, ObservationGate
...
def test_speech_segmenter_never_emits_silence_and_has_max_duration():
    vad = Segmenter(sample_rate=16000, silence_seconds=.3, max_seconds=1)
    silence = np.zeros(1600, dtype=np.float32)
    assert all(vad.feed(silence) is None for _ in range(20))
    result = None
    for _ in range(12):
        chunk = vad.feed(np.full(1600, .1, dtype=np.float32))
...
def test_speech_segmenter_emits_after_end_of_sentence():
    vad = Segmenter(sample_rate=16000, silence_seconds=.3)
    for _ in range(7):
        assert vad.feed(np.full(1600, .08, dtype=np.float32)) is None
